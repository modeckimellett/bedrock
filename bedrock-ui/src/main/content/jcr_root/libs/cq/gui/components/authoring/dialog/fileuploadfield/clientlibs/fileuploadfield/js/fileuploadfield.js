;(function ($, ns, channel, window, document, undefined) {
        ...
        $.ajax({
            ...
            if (fileRef) {
                if (self._isImageMimeType(fileRef)) {
                    thumbnailDom = self._createImageThumbnailDom(fileRef);
                } else {
                ...
                if (self._isImageMimeType(fileName)) {
                    thumbnailDom = self._createImageThumbnailDom(self.resourceURL + '/file');
                } else {
                    // display only file name
                    thumbnailDom = $("<p>" + fileName  + "</p>");
                }
            }
    ...
    ns.FileUploadField.prototype._bindEvents = function () {
        ...
        self.widget.$element.on("fileselected", function (event) {
            ...
            if (self._isImageMimeType(event.item.fileName)) {
        ...
        if (Granite.author) {
            channel
                .off("cq-asset-dragstart.fileuploadfield")
                .on("cq-asset-dragstart.fileuploadfield", function (event) {
                    self.widget.$element.addClass(_isActiveClass);
                })

                .off("cq-asset-dragend.fileuploadfield")
                .on("cq-asset-dragend.fileuploadfield", function (event) {
    ...
    ns.FileUploadField.prototype._isImageMimeType = function(fileName) {
        var mimeType = CUI.FileUpload.MimeTypes.getMimeTypeFromFileName(fileName.toLowerCase());
        return (mimeType !== undefined && mimeType.indexOf('image') === 0);
    };

    // when a dialog shows up, initialize the fileupload field
    $(document).on("foundation-contentloaded", function (event) {

        $(document).find(".cq-dialog .coral-FileUpload").each(function(){
        	var $element = $(this)
