...
class DefaultBasicNodeSpec extends BedrockSpec {
		...
		pageBuilder.content {
			citytechinc("CITYTECH, Inc.") {
				...
				externalPath: "http://www.reddit.com", multiValue: ["one", "two"]) {
					...
					burbon("sling:resourceType" : "yummy", fileReference: "/content/dam/image")
				}
			}
			ales {
				esb("ESB") {
	...
	def "get image source optional"() {
		...
		"/content/citytechinc/jcr:content/burbon"  | true
	}
	...
	def "get image source"() {
		...
		"/content/citytechinc/jcr:content/burbon"  | "/content/citytechinc/jcr:content/burbon.img.png"
	}
